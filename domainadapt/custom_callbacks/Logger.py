...
class Logger(object):
    ...
    def __init__(self, mylog_path="./log", mylog_name="training.log", myloss_names=["loss"],
                 mymetric_names=["Dice_Accuracy", "Accuracy"]):
    ...
    def to_csv(self, metric_array, epoch):
        if epoch == 0:
            ...
            test_c_lb = ['test_lb_' + t for t in train_c]
            test_c_un = ['test_un_' + t for t in train_c]
            df = pd.DataFrame(columns=train_c + val_c + test_c_lb + test_c_un)
